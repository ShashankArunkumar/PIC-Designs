...
- Always call the `detect_python_environment` tool before running or installing Python code; pass a workspace file URI when working in multi-root folders.
- Use the returned `pythonExec` and `envManager`; if `detectionSource=fileScan`, assume no active interpreter and rely on `projectManager` for installs only after user confirmation.
- Match installers: `uv` → `uv pip install --python <pythonExec> ...`; `poetry` → `poetry add ...`; `pdm` → `pdm add ...`; `hatch` → `hatch env run pip install ...`; `pipenv` → `pipenv install ...`; `conda` → `conda install -p <env> ...` or `conda env update -f environment.yml`; `pip` → `pip install -r requirements*.txt`.
- Do not create new environments unless the user explicitly asks; if `detectionSource=none`, ask the user how to proceed before installing anything.
...
- Using numpy methods (`np.max()`, `np.argmax()`, `np.min()`, etc.) directly on xarray DataArrays can cause errors. Always convert first: `data_np = data.values` before using numpy operations.
- Accessing xarray coordinates requires selecting a dataset first. For example, use `gaussian_beam.field_data.Ey.y.values` not `gaussian_beam.field_data.y.values`.
- `web.estimate_cost()` requires a `task_id`. Use `web.estimate_cost(job.task_id)` after creating a Job object, not `web.estimate_cost(simulation)`.
...
- Rotate — `tidy3d_rotate_viewer(viewer_id, direction)`: rotates the 3D view to match a specified direction.
- Show Structures — `tidy3d_show_structures(viewer_id, visibility)`: set per-structure visibility flags; order follows the viewer structure list.
- Capture — `tidy3d_capture(viewer_id)`: take a viewer screenshot and return the image payload.
- Python Env — `tidy3d_detect_python_environment(resource?)`: resolve interpreter/env/project managers; call before running or installing Python code.
